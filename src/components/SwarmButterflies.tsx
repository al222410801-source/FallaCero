...
const Butterfly = ({index, centerX, centerY, scale, delay, palette}: ButterflyProps) => {
  const color = palette[index % palette.length] || '#0ea5a4';
...
export const SwarmButterflies = ({
  ...
  const palette = [C.primary, C.accent, C.accentAlt].filter(Boolean);
