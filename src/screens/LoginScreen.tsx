...
// (theme tokens will be computed inside the component)
...
// Particle and CharacterPanel will be created inside the component to access theme tokens

// helper components will be created inside the component so they can use dynamic theme tokens
...
export default function LoginScreen({navigation}: {navigation?: any}) {
  ...
  // compute theme tokens and styles dynamically
  const {isDark} = useTheme();
  const C = getAppColors(isDark);
  const styles = makeStyles(C);

  // Particle (animated floating dots)
  ...
  // Character panel with decorative elements
  ...
  // FieldInput inside component to use dynamic styles/C
  const FieldInput = ({label, value, onChange, placeholder, secure = false, icon, delay = 0, onBlur, error}: {
  ...
  // Login button inside component
  const LoginBtn = ({onPress, title, disabled = false, loading = false}: {
    onPress: () => void; title: string; disabled?: boolean; loading?: boolean;
  }) => {
    ...
    return (
      <Animated.View style={{
        ...
        >
          ...
          <Text style={[styles.loginBtnText, {color: '#f5ead0'}]}>{title}</Text>
          <Text style={[styles.loginBtnArrow, {color: '#f5ead0'}]}>→</Text>
        </Pressable>
      </Animated.View>
    );
  };
            ...
            <View style={styles.centerColumn}>
              <Animated.View style={[
                ...
                <View style={styles.logoArea}>
                  ...
                  <Text style={styles.brandTitle}>FallaCero</Text>
                ...
                <View style={styles.loginForm}>
                  <FieldInput
                    ...
                    error={touched.username && !form.username.trim() ? 'Nombre de usuario requerido' : null}
                  />
                  <FieldInput
                    label="CONTRASEÑA"
                    value={form.password}
                    ...
                    error={touched.password && !form.password ? 'Contraseña requerida' : null}
                  />
...
const makeStyles = (C: any) => StyleSheet.create({
  ...
  // ── Personaje ───────────────────────────────────────────────────────────────
  characterPanel:   {alignItems: 'center', justifyContent: 'center', width: '100%', height: 420},
  characterScene:   {width: 360, height: 380, alignItems: 'center', justifyContent: 'center'},
  ...
  characterImg:     {width: 360, height: 360},
  ...
  // Layout: imagen a la derecha y formulario a la izquierda — espacio reducido
  sideRow: {flexDirection: 'row', alignItems: 'flex-start', justifyContent: 'center', gap: 0, width: '100%'},
  sideImageWrap: {width: 360, alignItems: 'center', justifyContent: 'center', marginLeft: -36},
  formWrapWide: {flex: 1, maxWidth: 520, marginRight: 0},
  centerColumn: {width: '100%', alignItems: 'center', gap: 12},
  characterContainer: {alignItems: 'center', marginTop: 12, marginBottom: 8},
