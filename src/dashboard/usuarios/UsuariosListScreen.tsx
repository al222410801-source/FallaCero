...
const UsuarioCard = ({usuario, onEditar, onEliminar}: {
  ...
  const containerStyle: any = {
    ...
    borderRadius: 16,
    overflow: 'hidden',
    backgroundColor: (C.accentAlt || C.primary) + '18',
    borderColor: C.border,
    borderWidth: 1,
  };

  const diagonalStyle: any = {
    position: 'absolute',
    top: -40,
    right: -60,
    width: 160,
    height: 260,
    backgroundColor: isDark ? '#1f2937' : C.card,
  ...
  return (
    <View style={containerStyle}>
      ...
      <View className="flex-row items-center mb-2 gap-3">
        ...
        <View style={{paddingHorizontal: 8, paddingVertical: 4, borderRadius: 999, backgroundColor: usuario.activo ? '#ecfdf5' : '#fee2e2'}}>
          <Text style={{fontSize: 12, fontWeight: '600', color: usuario.activo ? '#166534' : '#7f1d1d'}}>
      ...
      <View style={{flexDirection: 'row', marginTop: 12, gap: 8}}>
        <Pressable onPress={onEditar} style={{flex: 1, alignItems: 'center', paddingVertical: 10, borderRadius: 8, backgroundColor: C.primary}}>
          <Text style={{color: '#fff', fontSize: 14, fontWeight: '600'}}>✏️ Editar</Text>
        </Pressable>
        <Pressable onPress={onEliminar} style={{flex: 1, alignItems: 'center', paddingVertical: 10, borderRadius: 8, backgroundColor: C.destructive || '#ef4444'}}>
          <Text style={{color: '#fff', fontSize: 14, fontWeight: '600'}}>🗑️ Eliminar</Text>
        </Pressable>
      </View>
    </View>
  );
